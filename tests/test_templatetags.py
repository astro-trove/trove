...
class TestEventCandidateExtras:
    ...
    def test_bool_format_yesno_true(self):
        """Test _bool_format with truthy value."""
        from custom_code.templatetags.event_candidate_extras import _bool_format_yesno
        assert _bool_format_yesno(1.0) == "No"

    def test_bool_format_yesno_false(self):
        """Test _bool_format with falsy value."""
        from custom_code.templatetags.event_candidate_extras import _bool_format_yesno
        assert _bool_format_yesno(0.0) == "Yes"

    def test_bool_format_yesno_fractional(self):
        """Test _bool_format with fractional value."""
        from custom_code.templatetags.event_candidate_extras import _bool_format_yesno
        assert _bool_format_yesno(0.7) == "No"


class TestScoringToggles:
    """Tests for the photometry toggle lock in scoring_toggles."""

    EXTRAS = "custom_code.templatetags.event_candidate_extras"

    def _render_context(self, query="", target_id=None, scores_exist=False,
                        phot_method="trove", vet_all_allowed=True, superuser=False):
        from django.test import RequestFactory
        from custom_code.templatetags.event_candidate_extras import scoring_toggles

        request = RequestFactory().get("/eventcandidates/" + query)
        request.user = MagicMock(is_superuser=superuser)
        with patch(f"{self.EXTRAS}.kilonova_scores_exist", return_value=scores_exist) as exists, \
             patch(f"{self.EXTRAS}.vet_all_is_allowed", return_value=vet_all_allowed), \
             patch(f"{self.EXTRAS}.cache"), \
             patch("scoring.phot_method.get_phot_method", return_value=phot_method):
            ctx = scoring_toggles({"request": request}, target_id=target_id)
        return ctx, exists

    def test_event_without_scores_is_locked(self):
        ctx, exists = self._render_context("?nonlocalizedevent=7")
        exists.assert_called_once_with(nonlocalizedevent_id=7)
        assert ctx["kilonova_locked"] is True
        assert ctx["on_target_page"] is False
        assert "vetallchoice" in ctx["vet_url"]
        assert ctx["vet_url"].endswith("?vetting_method=KN&phot_method=kilonova")

    def test_locked_overrides_site_setting(self):
        # the site toggle may be on KilonovaSCORER from another event
        ctx, _ = self._render_context("?nonlocalizedevent=7", phot_method="kilonova")
        assert ctx["kilonova_locked"] is True
        assert ctx["is_kilonova"] is False

    def test_event_with_scores_is_unlocked(self):
        ctx, _ = self._render_context("?nonlocalizedevent=7", scores_exist=True,
                                      phot_method="kilonova")
        assert ctx["kilonova_locked"] is False
        assert ctx["is_kilonova"] is True

    def test_no_event_in_scope_is_unlocked(self):
        ctx, exists = self._render_context("")
        exists.assert_not_called()
        assert ctx["kilonova_locked"] is False
        assert ctx["vet_url"] is None

    def test_vet_all_cooldown_hides_link(self):
        ctx, _ = self._render_context("?nonlocalizedevent=7", vet_all_allowed=False)
        assert ctx["vet_all_on_cooldown"] is True
        assert ctx["vet_url"] is None

    def test_superuser_bypasses_cooldown(self):
        ctx, _ = self._render_context("?nonlocalizedevent=7", vet_all_allowed=False,
                                      superuser=True)
        assert ctx["vet_all_on_cooldown"] is False
        assert ctx["vet_url"] is not None

    def test_target_page_scopes_to_target(self):
        ctx, exists = self._render_context("?nonlocalizedevent=S251112cm", target_id=42)
        exists.assert_called_once_with(target_id=42)
        assert ctx["kilonova_locked"] is True
        assert ctx["on_target_page"] is True
        assert "/targets/42/vetchoice/" in ctx["vet_url"]
